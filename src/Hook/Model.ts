...
export default abstract class Model {
  ...
  constructor(appInstance: ComponentInternalInstance | null, table: string) {
    ...
    this.sqliteService =
      this.appInstance?.appContext.app.config.globalProperties.$sqliteServ;
  }
  ...
  async create(data: Record<string, any>): Promise<number | undefined> {
    await this.init();
    const { keys, values } = this.checkTimestamp(
      Object.keys(data),
      Object.values(data)
    );
  ...
  //Récuperation du dernier enregistrement
  async getMaxId(column = 'id'): Promise<number | null> {
    await this.init();
    const query = `SELECT MAX(${column}) as maxId FROM ${this.table}`;
    const result = await this.db.query(query);
    await this.close();
    return result.values?.[0]?.maxId ?? null;
  }
}
